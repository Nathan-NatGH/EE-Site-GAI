...
// First, let's keep the user's HTML inside `explanationMarkdown` directly, 
// because `react-markdown` supports raw HTML via the `rehype-raw` plugin. 
// OR we can just use dangerouslySetInnerHTML if we must, but standard react-markdown might strip divs. 
// Instead, I will write it directly as `explanationHTML` or keep `explanationMarkdown` and rely on user's requested integration.
// Actually, it's safer to just provide the exact text. Let's output it to a new file `src/data/a1.ts` 
// and update `lessons.ts` to export it.
...
// Since the user wants a modular approach like a JSON file:
fs.writeFileSync('src/data/a1.json', JSON.stringify(mapped, null, 2));

const tsCode = \`import a1Data from './a1.json';
...
// Load modular data
(a1Data as unknown as LessonContent[]).forEach(lesson => {
  lessonsData[lesson.id] = lesson;
});
\`;

fs.writeFileSync('src/data/lessons.ts', tsCode);
